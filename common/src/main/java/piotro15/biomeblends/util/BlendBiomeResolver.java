...
public class BlendBiomeResolver {
    private static BiomeResolver makeResolver(ChunkAccess chunk, BoundingBox boundingBox, ServerLevel level) {
        return (x, y, z, sampler) -> {
            ...
            if (boundingBox.isInside(quartX, quartY, quartZ)) {
                ...
                if (oldBiome != null) {
//                    return (Holder<Biome>) level.registryAccess().registryOrThrow(Registries.BIOME).getHolderOrThrow(ResourceKey.create(Registries.BIOME, ResourceLocation.fromNamespaceAndPath("wasteland", "wasteland")));
                    return new Holder.Direct<>(level.registryAccess().registryOrThrow(Registries.BIOME).get(Biomes.PLAINS));
                }
            }
    ...
    public static void purifyBiome(ServerLevel level, BlockPos pos, int range) {
        ...
        for (ChunkAccess chunk : chunks) {
            chunk.fillBiomesFromNoise(makeResolver(chunk, boundingBox, level), level.getChunkSource().randomState().sampler());
